...
float4 main(float2 inputVertex : POSITION) : SV_POSITION
{
    ...
    float4 alt = mul(float4(inputVertex, 0, 1),worldo);
	return alt;
}
)";
// Simple Pixel Shader
...
public:
	...
	struct SHADER_VARS
	{
		GW::MATH::GMATRIXF TheWorldo = GW::MATH::GIdentityMatrixF;//jojo:)
		GW::MATH::GMATRIXF pad;
	};
	// TODO: Part 4a
	struct VERTEX
	{
	...
	Renderer(GW::SYSTEM::GWindow _win, GW::GRAPHICS::GVulkanSurface _vlk)
	{
		...
		VERTEXholder plus;
		plus = BigPlusGen(x, y, 1);
		// TODO: Part 1c
		// TODO: Part 3c
		// TODO: Part 4a
		// Transfer triangle data to the vertex buffer. (staging would be prefered here)
		GvkHelper::create_buffer(physicalDevice, device, sizeof(plus.ver),
			VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
			VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &vertexHandle, &vertexData);
		GvkHelper::write_to_buffer(device, vertexData, plus.ver, sizeof(plus.ver));
		// TODO: Part 3c
	...
	void Render()
	{
		...
		proxy.RotateZGlobalF(matrix, 1, jojo.TheWorldo);
		...
		// TODO: Part 2d
		vkCmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, 64, &jojo);
		// now we can draw
	...
	VERTEXholder BigPlusGen(float x[], float y[], int size)
	{
		VERTEXholder plus;
		for (size_t i = 0; i <= 12; i++)
		{
			plus.ver[i].X = x[i] / size;
			plus.ver[i].Y = y[i] / size;
